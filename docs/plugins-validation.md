...
- 修复未登记旧程序目录导致包校验误报：一致清单/文件摘要且无额外文件时恢复登记，失败写注册表不删除复用目录。23项宿主插件测试、3项管理API测试、4项Provider扩展测试、17项架构检查与严格Clippy通过（沿用result_large_err例外）。
- 实际18087使用公开旧版v0.2.1压缩包完成恢复安装，复现条件是该版本程序目录残留且注册表无记录。验证后升级打标包0.2.3；因用户此前已卸载，安装保持默认停用，业务数据保留。
- 快捷对话0.3.0已迁到独立公开仓库，已启用。实际检查更新接口分别读取两仓库Release成功，返回当前/最新版本、发布页、附件及SHA256。
- 浏览器模拟验收：作者和仓库显示、新版本提示、Markdown标题/表格/代码复制及脚本/危险链接过滤通过；未执行付费推理。保留tar.gz直链安装，没有仓库源码安装流程。
- 上游PR使用独立codex/plugin-framework-pr分支，移除了fork版本号、部署历史与运维截图，提供独立接口/开发指南；部署分支与生产18086隔离。
