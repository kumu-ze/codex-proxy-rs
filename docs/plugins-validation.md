...
原页面使用 input + datalist，加载后自动填入第一项；浏览器按当前文本过滤建议，因此选中 codex-auto-review 后只显示该项。现改为原生完整 select，下拉始终保留全部模型；另外提供“手动输入模型名…”及独立输入框。重新加载保留仍存在的选择。

在实际 18087 页面验证：7 个模型始终可选，连续切换两项成功；重新加载保留选中项；自定义输入可显示和收起；390px 视口无横向溢出。JavaScript 语法检查、Rust 构建与原生进程工作流通过。未发送推理请求。本次仅升级 chat-test 0.2.2，宿主仍为 3.12.1-plugin.5，打标插件和用户配置未改。

# v3.12.1-plugin.5 与对话插件 0.2.1：模型列表恢复

2026-09-20 验证重点是“HTTP 200 却显示没有可用模型”，不是仅调整显示地址。

- 变更前实际请求已经是 `/v1/models`，返回 `200 / data:[]`；带 client_version 的原生目录返回 503 model_catalog_unavailable，账号目录刷新约 15 秒失败。
- 同一验证容器、同一日本代理 192.168.1.10:7893，SOCKS5 本地 DNS 模式连接超时；SOCKS5H 远端 DNS 模式可连接模型服务。保留代理主机、端口、名称及绑定，只在验证实例将该保存代理的 scheme 改为 socks5h，其绑定的两个账号同步生效。
- 使用独立诊断进程复用 Provider 的实际目录读取路径，不启动业务 worker、不提供新网关服务；仅记录阶段和耗时。认证模型请求最终返回 200，完整响应约 399 KB，约 9.7 秒完成。诊断曾放宽其自身目录预算便于测量，但成功时间低于生产原预算；诊断代码已移除，生产代码的 15 秒预算未变。
- 目录恢复后，标准 `/v1/models` 与 `/v1/models?client_version=0.153.4` 均实际返回 7 个模型。没有填充硬编码模型列表，没有修改 Key 权限或账号分组，也没有发起付费推理测试。
- 实际浏览器已确认 API 地址为当前站点 `/v1`，网络请求精确为 `/v1/models`，点击加载后显示“已加载 7 个模型”。前端 ESLint、类型检查与构建通过；插件严格 Clippy、握手/页面与密钥边界工作流通过。
- 页面将站点地址显示为 API 地址（带 /v1）；请求路径保持各自固定端点。旧 ticket 分支远端已是上游 3.12.1 基线，旧名 codex/rs-3.11-ticket 仅为历史分支名，README 已注明运行构建 v3.12.1-ticket.1。

![模型加载恢复（Key 选择区域已遮挡）](images/plugins/chat-models-restored.png)
