...
manifest={'id':'chat-test','version':'0.2.2','apiVersion':1,'menuLabel':'对话测试','capabilities':['ui.chat'],'executable':'worker','files':{'worker':hashlib.sha256((output/'worker').read_bytes()).hexdigest()}}
(output/'plugin.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
with tarfile.open(output.parent/'rs-chat-test-0.2.2-linux-x64.tar.gz','w:gz') as tar:
